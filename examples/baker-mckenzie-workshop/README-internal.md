<!-- INTERNAL prep copy (talk tracks, discovery questions, reference-env specifics). Customer-facing version is README.md. -->
...
## Policies and controls enforced by these deployments (day-2 reference)

Everything below is enforced by the two stacks as deployed - not aspirational.
Effects/values shown are the ones live in the demo `applied-ai` sandbox. Speak to
these as "what the platform enforces, regardless of who the developer is."

### A. Model governance - Azure Policy on the sandbox resource group

- **Policy:** built-in *"[Preview]: Azure Machine Learning Deployments should only
  use approved Registry Models"* (`12e5dd16-d201-47ff-849b-8454061c293d`). Foundry
  model deployments go through the AML resource provider, so this governs Foundry.
- **Scope:** the sandbox resource group (`rg-ailz-applied-*`) - assigned per sandbox.
- **What it restricts:** model deployments to **approved publishers only**, derived
  from `allowed_models` (live: `gpt-4.1`, `text-embedding-3-large`). Optionally
  pinnable to exact model asset IDs/versions via `allowedAssetIds`.
- **Effect (posture toggle `enforce_model_policy_effect`):** live value is **`Audit`**
  (audit-first: teams learn what they need, violations are flagged not blocked). Flip
  to **`Deny`** to hard-block unapproved models once the catalog is agreed. `Disabled`
  removes it. **This is the "governed model catalog" control** - the one to demo.
- **Talking point:** at Baker scale this same class of policy is assigned at the
  **management group**, so it applies to every sandbox subscription and overrides even
  a subscription Owner. Governance is enforced by the platform, not by trusting teams.

### B. Model deployment shape - Data residency / SKU

- Chat model deployed as **`DataZoneStandard`** (`model_deployment_type`) - keeps
  inference within the geographic data zone (EU/US), the ethical-wall answer for
  client-confidential matters. Embeddings pinned to `GlobalStandard` (broadest
  availability). Deployment SKU is governed by the module, not left to the team.

### C. Content safety (Responsible AI)

- **`enable_content_safety = true`** on the sandbox - the Foundry account is
  provisioned with Content Safety available (prompt shields, groundedness,
  protected-material checks) as the baseline RAI control surface for the pilot.

### D. Network / data-plane controls (enforced by the module posture)

- **Foundry is private-only** (Terraform module hardcodes private endpoints, no
  public data plane) - posture A'. Search, Cosmos, Storage, Key Vault all reach via
  private endpoints too.
- **Centralized Private DNS** (hub) resolves every sandbox privately; the DINE
  initiative auto-registers new private endpoints (see the centralized-DNS section).
- **Managed identity, not keys:** the gateway authenticates to Foundry with its
  system-assigned identity (`Cognitive Services OpenAI User`); developers get
  **Azure AI Developer** RBAC on their project (`developer_group_object_id`), so
  access is Entra-gated, least-privilege, and auditable.

### E. Cost guardrail - resource-group budget

- **`azurerm_consumption_budget_resource_group`** per sandbox (live: `budget-applied-ai`,
  **$500/month**, monthly grain).
- **Alerts:** 80% actual and 100% actual to `budget_alert_emails`. **Production**
  posture adds a **100% forecasted** alert (posture toggle). This is the per-sandbox
  half of showback.

### F. Gateway governance policy (Stack A) - enforced per team at the AI gateway

Applied as the APIM API policy on each team's API (`azurerm_api_management_api_policy`):

- **`authentication-managed-identity`** - APIM authenticates onward to the private
  Foundry with its MI (`resource=https://cognitiveservices.azure.com`). Callers never
  hold a Foundry key.
- **`set-backend-service`** - routes the team's API to *their* Foundry backend only
  (team isolation at the gateway).
- **`azure-openai-token-limit`** - **per-team fair-use ceiling**, counter keyed on the
  team's APIM subscription (`counter-key = subscription.id`), computed from
  `monthly_token_limit` (live: 200,000/mo -> ~1,000 tokens/min). Over-limit returns
  **HTTP 429** - demoed live. One noisy team cannot starve the others.
- **`azure-openai-emit-token-metric`** - emits **per-team token metrics** (dimensions
  `team`, `api`) to the hub Log Analytics workspace for **showback**.
- **`subscription_required = true`** on the API/product - the **subscription key is the
  per-team access boundary** (no key -> HTTP 401, demoed live).
- **Diagnostics:** APIM `GatewayLogs` + `AllMetrics` shipped to the hub LAW.

### Governance posture summary (what to say on day 2)

| Control | Where | Live effect | Production move |
| --- | --- | --- | --- |
| Approved models only | Azure Policy on sandbox RG | **Audit** | Flip to **Deny**; assign at mgmt group |
...
| Private networking + DNS | Module posture + hub DNS | **Private-only** | DINE at mgmt group |
| Identity, not keys | RBAC + APIM MI | **Enforced** | PIM/CA on the groups |
| Per-team token ceiling | APIM policy | **~1k TPM -> 429** | Tune per team/SLA |
| Per-team showback | APIM token metrics -> LAW | **Enabled** | Wire to cost dashboard |
| Spend guardrail | RG budget + alerts | **$500/mo, 80/100%** | Forecast alert in prod |
